$(document).ready(function () {
    $("#playdifficile").click(() => startGame(1.2, 1100));
    $("#playmedio").click(() => startGame(3.5, 3400));
    $("#playfacile").click(() => startGame(6, 5800));
});
...
const startGame = (travelTime, destroyAsteroidTimeout) => {
    $("main").empty();
    var pScore = $("<p>").addClass("position-absolute top-0 start-0 text-white m-5");
    pScore.text("score : 0");
    $("main").append(pScore);
    scoreBoardTimer(pScore);
    generateAsteroid(travelTime, destroyAsteroidTimeout);
...
function generateAsteroid(travelTime, destroyAsteroidTimeout) {
